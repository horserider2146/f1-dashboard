...
A full-stack Formula 1 data analytics platform built with **FastAPI**, **Streamlit**, and **FastF1**. Provides race telemetry, strategy analysis, machine learning predictions, and university-level statistical analysis — all in an interactive web dashboard.
...
| Page                     | What it does                                                       |
| ------------------------ | ------------------------------------------------------------------ |
| **Race Data**            | Lap times, position history, fastest laps, track map and animation |
| **Telemetry**            | Speed traces and telemetry interpretation                          |
...
Complete race overview — lap times table, fastest laps per driver, circuit track map + multi-driver animation, and an interactive position history chart showing every position change lap by lap.

### Telemetry

- **Speed Trace** — speed (km/h) across every meter of a lap
- **DRS Context** — telemetry-based interpretation of drag-reduction activation behavior
